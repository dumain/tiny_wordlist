...
'evil','lard','you','your','ogle','rue','huff'
])
...
#wordlist_length = 6**5 # regular diceware uses 5 dice for 6^5 = 7,776 words
wordlist_length = 6**6 # this version uses 6 dice for 6^6 = 46,656 words

# repeat the list a few times, alternating between lower, title, reverse title, and all caps
new_words = []
capitalize = 0
while len(new_words) < wordlist_length:
	if capitalize == 0:
		new_words = new_words + words
	elif capitalize == 1:
		new_words = new_words + [x.title() for x in words]
	#elif capitalize == 2:
	#	new_words = new_words + [x[-1::-1].title()[-1::-1] for x in words] # reverse title case: "abc" -> "abC"
	else:
		new_words = new_words + [x.upper() for x in words]
		capitalize = -1
	capitalize += 1

words = new_words[0:wordlist_length]

# create repeated list of 7,776 numbers/specials
#num_specials = ['1','2','3','4','5','6','7','8','9','0','.','-','+',':','?','!','@','#','$','%','^','&','*','~']
# the six-die wordlist length is evenly divisible by the above 24 digits+specials list, so remove a special here
num_specials = ['1','2','3','4','5','6','7','8','9','0','.','-','+',':','?','!','@','#','$','%','&','*','~']
new_num_specials = []
while len(new_num_specials) < wordlist_length/2:
	new_num_specials = new_num_specials + num_specials
num_specials = new_num_specials[0:wordlist_length]


# zip the words and num/specials together: word+char
zipped_words = []
zipped_words = [f"{w}{ns}" for w,ns in zip(words, num_specials)]
# zip the words and num/specials together: char+word
zipped_words = zipped_words + [f"{ns}{w}" for w,ns in zip(words, num_specials)]

words = zipped_words[0:wordlist_length]

if len(words) != wordlist_length:
	print(f"{wordlist_length:,} words are required (have {len(words):,})")
	sys.exit(1)

# make sure there are no repeats, since we repeated the words and other chars before zipping them
test_set = set(words)
if len(test_set) != wordlist_length:
	print(f"{wordlist_length:,} words are required (have {len(test_set):,} after checking for repeats)")
	sys.exit(1)
				...
				for e in range(1, 7):
					for f in range(1, 7):
						...
						if use_line_number_column:
							print("%5d\t%d%d%d%d%d%d\t%s" % (line_num, a, b, c, d, e, f, words[line_num-1]))
						else:
							print("%d%d%d%d%d%d\t%s" % (a, b, c, d, e, f, words[line_num-1]))
